...
Defines the geometric primitives the TUI uses to address screen regions and the constraint-based allocator that splits a parent rect into contiguous sub-rects. Provides `Rect`, the `Constraint` union (fixed/percentage/ratio/min/max/fill), and the horizontal/vertical split operators that resolve constraints with deterministic rounding.
