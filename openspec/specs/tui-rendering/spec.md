...
Defines the in-memory canvas model the TUI uses to compose a frame and the diff-to-ANSI translator that emits only the cells which changed since the previous frame. Covers `Style` / `Color`, the wide-grapheme-aware `Cell`, the row-major `Buffer` with bounds-clipping writes, the `Frame` wrapper, and the minimal-write `diff_to_ansi` driver.
